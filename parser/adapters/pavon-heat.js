...
// августа» — сразу по ГОРОДУ (212+ домов на ~30 улицах: Сатпаева, Лермонтова, Айманова,
// Естая, Торайгырова и т.д.), но по факту воду дали раньше срока везде. Сама статья на
// сайте задним числом не обновляется, поэтому парсер продолжал бы каждый час выдумывать
// это отключение заново по ВСЕМ этим адресам. Раз пользователь подтвердил, что во всём
// городе уже нет отключений по этому конкретному обещанию — глушим по ДАТЕ подключения
// (любая секция статьи с end = 20 августа текущего года), а не по адресам: список улиц
// слишком большой и хрупкий для поддержки вручную. Снять исключение можно просто удалив
// блок STALE_END_DATES, когда появится новая актуальная статья с тем же числом.
const STALE_END_DATES = new Set([
  Date.UTC(new Date(NOW).getUTCFullYear(), 7, 20, 23, 59), // 20 августа (месяц 7 = август, 0-indexed)
]);
...
async function finishPavonHeat(parsed) {
  ...
  for (const sec of parsed) {
    ...
    if (STALE_END_DATES.has(end)) continue;
