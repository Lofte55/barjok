...
const STYLE = `
  ...
  * { box-sizing: border-box; }
  ...
  .actions input.dur-custom-val
    { margin-right: 6px; margin-bottom: 4px; padding: 6px 8px; font-size: 12.5px; vertical-align: middle;
      background: #0f1115; border: 1px solid #2c3140; color: #e7e9ee; border-radius: 8px; width: 60px; }
  /* Срок в строке таблицы (durationSelectHtml()) — тот же .csel, что и везде
     в админке, но компактнее: базовый .csel-btn (min-width:140px, заточен под
     фильтры сверху) слишком широк для тесной ячейки .actions. */
  .actions .dur-csel { margin-right: 6px; margin-bottom: 4px; vertical-align: middle; }
  .actions .dur-csel .csel-btn { min-width: 96px; padding: 6px 8px; font-size: 12.5px; }
  .actions .dur-csel .csel-menu { font-size: 12.5px; }
  ...
  @media (max-width: 760px) {
    ...
    tbody tr td.actions button, tbody tr td.actions input.dur-custom-val { font-size: 13px; }
    tbody tr td.actions .dur-csel .csel-btn { font-size: 13px; }
  }
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  // ⚠️ На window — страничный <script> (api/admin.js) вызывает buildCsel() САМ
  // для селектов, которые появляются ПОСЛЕ этого DOMContentLoaded (динамически
  // рендерятся в строках таблицы, см. durationSelectHtml()). Без этого их
  // <select> остаётся системным — buildCsel сканирует DOM только один раз здесь.
  window.buildCsel = buildCsel;
