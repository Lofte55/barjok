...
/*
 * ⚠️⚠️ «ова»/«ева» УБРАНЫ из альтернативы (были здесь изначально) — регэксп жадно
 * снимал ТРИ буквы («-ова»), а не только окончание падежа («-а»), из-за чего русская
 * фамилия-улица «Лермонтова» стеммилась в «лермонт», а не в «лермонтов» — и переставала
 * совпадать с казахской именительной формой «Лермонтов» (из «Лермонтов көшесі»), которая
 * не режется вовсе (оканчивается на согласную). Найдено на живом баге: OSM тегает один
 * дом на улице казахским вариантом, а остальные — русским («Лермонтова, 55» существует
 * в реестре ТОЛЬКО как «Лермонтов көшесі, 55»), и exact-match по неправильному ключу
 * «лермонт» перехватывал housesOnStreet() раньше, чем срабатывал soft-match — этот
 * единственный дом был НЕВИДИМ для любого запроса на русском написании улицы.
 * Достаточно снимать только гласную «-а»/«-я» (уже есть в списке ниже) — «-ов-»/«-ев-»
 * это часть корня фамилии, а не падежное окончание, резать её нельзя.
 * Проверено на всём реестре (737 улиц): 6 реальных RU/KZ пар слились правильно
 * (Лермонтова, Ломова, Айманова, Бекмаханова, Дерова, Исиналиева), новых ложных
 * слияний между РАЗНЫМИ улицами не возникло.
 */
function stem(w) {
  return w.replace(/(ского|ская|ские|ина|ыны|ая|ой|ый|ого|ому|ым|ых|а|я|ы|и|о|е|у)$/i, '');
}
