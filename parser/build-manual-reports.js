 ...
 *  4) для сетевых ресурсов (вода/отопление/газ) добавляет СОСЕДНИЕ дома на той же улице
 *     (номер ±3) — пользователь подтвердил, что при подъездных отключениях соседние дома
 *     того же дома/квартала тоже без ГВС. ЭЛЕКТРИЧЕСТВО ИСКЛЮЧЕНО: плановые отключения
 *     электричества публикуются по точному списку домов, и достраивать соседние дома
 *     нельзя — это может исказить официальный график.
...
(async () => {
  ...
  for (const r of REPORTS) {
    ...
    // Электричество исключено: плановые отключения не должны «нарушаться» достройкой
    // соседних домов, которых не было в официальном списке.
    const n = houseNum(r.house);
    if (r.resource !== 'electricity' && !Number.isNaN(n) && loc.list) {
